...
int main(int argc, char **argv) {
    ...
    ptsTo(_imopVarPre160) := [nullCell];
    ptsTo(argv) := [heapCell#0];
    ptsTo(A_imopVar0) := [nullCell];
    ]*/
    double ( *A_imopVar0 )[1000 + 0];
    /*OUT: [ptsTo(__stderrp) := [nullCell];
    ptsTo(suboptarg) := [nullCell];
    ptsTo(A) := [heapCell#1];
    ptsTo(_imopVarPre146) := [heapCell#1];
    ...
    ptsTo(argv) := [heapCell#0];
    ptsTo(A_imopVar0) := [nullCell];
    ptsTo(B_imopVar1) := [nullCell];
    ]*/
    double ( *B_imopVar1 )[1000 + 0];
    /*OUT: [ptsTo(__stderrp) := [nullCell];
    ptsTo(suboptarg) := [nullCell];
    ptsTo(A) := [heapCell#1];
    ptsTo(_imopVarPre146) := [heapCell#1];
...
#pragma omp for schedule(static) nowait
            ...
            for (i = 1; i < n - 1; i++) {
                ...
                for (j = 1; j < n - 1; j++) {
                    ...
                    B_imopVar1[i][j] = 0.2 * (A_imopVar0[i][j] + A_imopVar0[i][j - 1] + A_imopVar0[i][1 + j] + A_imopVar0[1 + i][j] + A_imopVar0[i - 1][j]);
                }
            }
#pragma omp for schedule(static) nowait
            /*OUT: [ptsTo(__stderrp) := [nullCell];
