...
#pragma omp master
            ...
            _imopVarPre276 = malloc(_imopVarPre275);
            clusters[0] = (float *) _imopVarPre276;
            for (i_imopVar79 = 1; i_imopVar79 < nclusters; i_imopVar79++) {
                clusters[i_imopVar79] = clusters[i_imopVar79 - 1] + nfeatures;
            }
            for (i_imopVar79 = 0; i_imopVar79 < nclusters; i_imopVar79++) {
                signed long int _imopVarPre278;
                _imopVarPre278 = random();
                int n = (int) _imopVarPre278 % npoints;
                for (j = 0; j < nfeatures; j++) {
                    clusters[i_imopVar79][j] = feature[n][j];
                }
            }
            for (i_imopVar79 = 0; i_imopVar79 < npoints; i_imopVar79++) {
                membership[i_imopVar79] = -1;
            }
            unsigned long int _imopVarPre281;
            void *_imopVarPre282;
            _imopVarPre281 = sizeof(int);
            ...
            new_centers[0] = (float *) _imopVarPre292;
            for (i_imopVar79 = 1; i_imopVar79 < nclusters; i_imopVar79++) {
                new_centers[i_imopVar79] = new_centers[i_imopVar79 - 1] + nfeatures;
            }
            if (!is_perform_atomic) {
                unsigned long int _imopVarPre295;
                void *_imopVarPre296;
                ...
                _imopVarPre302 = calloc(_imopVarPre301, _imopVarPre300);
                partial_new_centers_len[0] = (int *) _imopVarPre302;
                for (i_imopVar79 = 1; i_imopVar79 < nthreads; i_imopVar79++) {
                    partial_new_centers_len[i_imopVar79] = partial_new_centers_len[i_imopVar79 - 1] + nclusters;
                }
                unsigned long int _imopVarPre305;
                void *_imopVarPre306;
                _imopVarPre305 = nthreads * sizeof(float **);
                ...
                partial_new_centers[0] = (float **) _imopVarPre310;
                for (i_imopVar79 = 1; i_imopVar79 < nthreads; i_imopVar79++) {
                    partial_new_centers[i_imopVar79] = partial_new_centers[i_imopVar79 - 1] + nclusters;
                }
                for (i_imopVar79 = 0; i_imopVar79 < nthreads; i_imopVar79++) {
                    for (j = 0; j < nclusters; j++) {
                        unsigned long int _imopVarPre313;
                        void *_imopVarPre314;
                        _imopVarPre313 = sizeof(float);
                        _imopVarPre314 = calloc(nfeatures, _imopVarPre313);
                        partial_new_centers[i_imopVar79][j] = (float *) _imopVarPre314;
                    }
                }
            }
            if (_debug) {
            ...
            do {
                delta = 0.0;
                if (is_perform_atomic) {
#pragma omp parallel private(i_imopVar79, j, index) firstprivate(npoints, nclusters, nfeatures) shared(feature, clusters, membership, new_centers, new_centers_len)
                    {
#pragma omp for schedule(static) reduction(+:delta) nowait
                        for (i_imopVar79 = 0; i_imopVar79 < npoints; i_imopVar79++) {
                            float *_imopVarPre316;
                            int _imopVarPre317;
                            _imopVarPre316 = feature[i_imopVar79];
                            _imopVarPre317 = find_nearest_point(_imopVarPre316, nfeatures, clusters, nclusters);
                            index = _imopVarPre317;
                            if (membership[i_imopVar79] != index) {
                                delta += 1.0;
                            }
                            membership[i_imopVar79] = index;
// #pragma omp dummyFlush ATOMIC_START
#pragma omp atomic
                            new_centers_len[index]++;
// #pragma omp dummyFlush ATOMIC_END
...
// #pragma omp dummyFlush ATOMIC_START
#pragma omp atomic
                                new_centers[index][j] += feature[i_imopVar79][j];
// #pragma omp dummyFlush ATOMIC_END
                            }
                        }
                    }
...
#pragma omp parallel shared(feature, clusters, membership, partial_new_centers, partial_new_centers_len)
                    {
                        ...
                        _imopVarPre318 = omp_get_thread_num();
                        int tid = _imopVarPre318;
#pragma omp for private(i_imopVar79, j, index) firstprivate(npoints, nclusters, nfeatures) schedule(static) reduction(+:delta) nowait
                        for (i_imopVar79 = 0; i_imopVar79 < npoints; i_imopVar79++) {
                            float *_imopVarPre320;
                            int _imopVarPre321;
                            _imopVarPre320 = feature[i_imopVar79];
                            _imopVarPre321 = find_nearest_point(_imopVarPre320, nfeatures, clusters, nclusters);
                            index = _imopVarPre321;
                            if (membership[i_imopVar79] != index) {
                                delta += 1.0;
                            ...
                            partial_new_centers_len[tid][index]++;
                            for (j = 0; j < nfeatures; j++) {
                                partial_new_centers[tid][index][j] += feature[i_imopVar79][j];
                            }
                        }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
#pragma omp master
                        {
                            for (i_imopVar79 = 0; i_imopVar79 < nclusters; i_imopVar79++) {
                                for (j = 0; j < nthreads; j++) {
                                    new_centers_len[i_imopVar79] += partial_new_centers_len[j][i_imopVar79];
                                    partial_new_centers_len[j][i_imopVar79] = 0.0;
                                    for (k = 0; k < nfeatures; k++) {
                                        new_centers[i_imopVar79][k] += partial_new_centers[j][i_imopVar79][k];
                                        partial_new_centers[j][i_imopVar79][k] = 0.0;
                                    }
                                }
                            }
                        }
                ...
                for (i_imopVar79 = 0; i_imopVar79 < nclusters; i_imopVar79++) {
                    for (j = 0; j < nfeatures; j++) {
                        if (new_centers_len[i_imopVar79] > 0) {
                            clusters[i_imopVar79][j] = new_centers[i_imopVar79][j] / new_centers_len[i_imopVar79];
                        }
                        new_centers[i_imopVar79][j] = 0.0;
                    }
                    new_centers_len[i_imopVar79] = 0;
                }
                delta /= npoints;
                _imopVarPre323 = delta > threshold;
                if (_imopVarPre323) {
        ...
        if (is_perform_valid) {
            ...
            int nfeatures;
            int npoints;
            float **cluster_centres_imopVar75;
            feature = attributes;
            nfeatures = numAttributes;
            npoints = numObjects;
            cluster_centres_imopVar75 = tmp_cluster_centres;
            int i_imopVar77;
            int j;
            int nearest_cluster;
            int *numpoints_in_clusters;
            ...
            float sqd_dist;
            float memb_fuzzyq;
            float membership_imopVar78;
            if (nclusters < 2) {
                _imopVarPre353 = 3.40282346638528859812e+38F;
                goto fuzzy_validity_imopVar76;
            }
#pragma omp parallel shared(feature, cluster_centres_imopVar75, numpoints_in_clusters) firstprivate(npoints, nfeatures, nclusters) private(i_imopVar77, nearest_cluster)
            {
                unsigned long int _imopVarPre244;
                void *_imopVarPre245;
#pragma omp master
...
#pragma omp barrier
#pragma omp for schedule(static) nowait
                for (i_imopVar77 = 0; i_imopVar77 < npoints; i_imopVar77++) {
                    float *_imopVarPre247;
                    int _imopVarPre248;
                    _imopVarPre247 = feature[i_imopVar77];
                    _imopVarPre248 = find_nearest_point(_imopVarPre247, nfeatures, cluster_centres_imopVar75, nclusters);
                    nearest_cluster = _imopVarPre248;
// #pragma omp dummyFlush ATOMIC_START
#pragma omp atomic
                    numpoints_in_clusters[nearest_cluster]++;
...
#pragma omp parallel shared(numpoints_in_clusters) firstprivate(nclusters) private(i_imopVar77)
            {
                unsigned long int _imopVarPre251;
                void *_imopVarPre252;
#pragma omp for schedule(static) reduction(+:penalty) nowait
                for (i_imopVar77 = 0; i_imopVar77 < nclusters; i_imopVar77++) {
                    penalty += 1.0 / (1.0 + ((float) numpoints_in_clusters[i_imopVar77]));
                }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
#pragma omp master
...
#pragma omp parallel firstprivate(npoints, nclusters, nfeatures, fuzzyq)
            {
                sum_fuzzy_members(feature, npoints, cluster_centres_imopVar75, nclusters, nfeatures, fuzzyq, sum_points);
            }
#pragma omp parallel shared(feature, cluster_centres_imopVar75) firstprivate(npoints, nclusters, nfeatures, fuzzyq) private(i_imopVar77, j, sqd_dist, memb_fuzzyq, membership_imopVar78)
            {
#pragma omp for schedule(static) reduction(+:valid_sum) nowait
                for (j = 0; j < npoints; j++) {
                    for (i_imopVar77 = 0; i_imopVar77 < nclusters; i_imopVar77++) {
                        ...
                        float *_imopVarPre256;
                        float _imopVarPre257;
                        _imopVarPre255 = cluster_centres_imopVar75[i_imopVar77];
                        _imopVarPre256 = feature[j];
                        _imopVarPre257 = euclid_dist_2(_imopVarPre256, _imopVarPre255, nfeatures);
                        sqd_dist = _imopVarPre257;
                        double _imopVarPre261;
                        ...
                        _imopVarPre262 = 1.0 / (sqd_dist + 0.00001);
                        _imopVarPre263 = pow(_imopVarPre262, _imopVarPre261);
                        membership_imopVar78 = _imopVarPre263 / (sum_points[j] + 0.00001);
                        memb_fuzzyq = pow(membership_imopVar78, fuzzyq);
                        valid_sum += sqd_dist * memb_fuzzyq;
                    }
                }
// #pragma omp dummyFlush BARRIER_START
...
#pragma omp master
                {
                    free(sum_points);
                    min_dist = 0.0;
                    for (i_imopVar77 = 0; i_imopVar77 < nclusters - 1; i_imopVar77++) {
                        float this_cluster_min_dist = 3.40282346638528859812e+38F;
                        for (j = i_imopVar77 + 1; j < nclusters; j++) {
                            float tmp_dist;
                            float *_imopVarPre266;
                            float *_imopVarPre267;
                            float _imopVarPre268;
                            _imopVarPre266 = cluster_centres_imopVar75[j];
                            _imopVarPre267 = cluster_centres_imopVar75[i_imopVar77];
                            _imopVarPre268 = multid_feuclid_dist(_imopVarPre267, _imopVarPre266, nfeatures);
                            tmp_dist = _imopVarPre268;
                            if (tmp_dist < this_cluster_min_dist) {
                                this_cluster_min_dist = tmp_dist;
