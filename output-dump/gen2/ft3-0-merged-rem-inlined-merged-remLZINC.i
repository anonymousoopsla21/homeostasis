...
int main(int argc, char **argv) {
    ...
    int kk;
    double ap;
#pragma omp parallel default(shared) private(i_imopVar95, j, k, ii, ii2, jj, ij2, kk)
    {
#pragma omp for nowait
        for (i_imopVar95 = 0; i_imopVar95 < dims[2][0]; i_imopVar95++) {
            ii = (i_imopVar95 + 1 + xstart[2] - 2 + 64 / 2) % 64 - 64 / 2;
            ii2 = ii * ii;
            for (j = 0; j < dims[2][1]; j++) {
                ...
                for (k = 0; k < dims[2][2]; k++) {
                    ...
                    indexmap[k][j][i_imopVar95] = kk * kk + ij2;
                }
            }
        }
    }
    ...
    ex[1] = _imopVarPre217;
    for (i_imopVar95 = 2; i_imopVar95 <= (6 * (64 * 64 / 4 + 64 * 64 / 4 + 64 * 64 / 4)); i_imopVar95++) {
        ex[i_imopVar95] = ex[i_imopVar95 - 1] * ex[1];
    }
    int ( *_imopVarPre147 );
...
#pragma omp master
            {
                for (i = 0; i < 3; i++) {
                    int _imopVarPre250;
                    int _imopVarPre251;
                    _imopVarPre250 = d_imopVar81[i];
                    _imopVarPre251 = ilog2(_imopVarPre250);
                    logd_imopVar84[i] = _imopVarPre251;
                }
            }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
...
#pragma omp for nowait
            for (k = 0; k < d_imopVar81[2]; k++) {
                for (ii = 0; ii <= d_imopVar81[0] - fftblock; ii += fftblock) {
                    for (j = 0; j < d_imopVar81[1]; j++) {
                        for (i = 0; i < fftblock; i++) {
                            y0[j][i].real = x_imopVar82[k][j][i + ii].real;
                            y0[j][i].imag = x_imopVar82[k][j][i + ii].imag;
                        }
                    }
                    int _imopVarPre254;
                    int _imopVarPre255;
                    _imopVarPre254 = d_imopVar81[1];
                    _imopVarPre255 = logd_imopVar84[1];
                    cfftz(is_imopVar80, _imopVarPre255, _imopVarPre254, y0, y1);
                    for (j = 0; j < d_imopVar81[1]; j++) {
                        for (i = 0; i < fftblock; i++) {
                            xout_imopVar83[k][j][i + ii].real = y0[j][i].real;
                            xout_imopVar83[k][j][i + ii].imag = y0[j][i].imag;
                        }
                    }
                }
            }
...
#pragma omp for nowait
        ...
        int is_imopVar75;
        int *d_imopVar76;
        struct stUn_imopVarPre11 ( *x_imopVar77 )[64][64];
        struct stUn_imopVarPre11 ( *xout_imopVar78 )[64][64];
        is_imopVar75 = _imopVarPre233;
        d_imopVar76 = _imopVarPre232;
        x_imopVar77 = x1;
        xout_imopVar78 = x1;
        int logd_imopVar79[3];
#pragma omp parallel default(shared) shared(is_imopVar75)
        {
            int i;
            int j;
            int k;
...
#pragma omp master
            {
                for (i = 0; i < 3; i++) {
                    int _imopVarPre257;
                    int _imopVarPre258;
                    _imopVarPre257 = d_imopVar76[i];
                    _imopVarPre258 = ilog2(_imopVarPre257);
                    logd_imopVar79[i] = _imopVarPre258;
                }
            }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
            ...
            dcomplex y1[64][18];
#pragma omp for nowait
            for (j = 0; j < d_imopVar76[1]; j++) {
                for (ii = 0; ii <= d_imopVar76[0] - fftblock; ii += fftblock) {
                    for (k = 0; k < d_imopVar76[2]; k++) {
                        for (i = 0; i < fftblock; i++) {
                            y0[k][i].real = x_imopVar77[k][j][i + ii].real;
                            y0[k][i].imag = x_imopVar77[k][j][i + ii].imag;
                        }
                    }
                    int _imopVarPre261;
                    int _imopVarPre262;
                    _imopVarPre261 = d_imopVar76[2];
                    _imopVarPre262 = logd_imopVar79[2];
                    cfftz(is_imopVar75, _imopVarPre262, _imopVarPre261, y0, y1);
                    for (k = 0; k < d_imopVar76[2]; k++) {
                        for (i = 0; i < fftblock; i++) {
                            xout_imopVar78[k][j][i + ii].real = y0[k][i].real;
                            xout_imopVar78[k][j][i + ii].imag = y0[k][i].imag;
                        }
                    }
                }
            }
...
#pragma omp for nowait
        ...
        is_imopVar85 = _imopVarPre241;
        d_imopVar86 = _imopVarPre240;
        x_imopVar87 = x1;
        xout_imopVar88 = x2;
...
#pragma omp master
            {
                for (i = 0; i < 3; i++) {
                    int _imopVarPre243;
                    int _imopVarPre244;
                    _imopVarPre243 = d_imopVar86[i];
                    _imopVarPre244 = ilog2(_imopVarPre243);
                    logd_imopVar89[i] = _imopVarPre244;
                }
            }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
...
#pragma omp for nowait
            for (k = 0; k < d_imopVar86[2]; k++) {
                for (jj = 0; jj <= d_imopVar86[1] - fftblock; jj += fftblock) {
                    for (j = 0; j < fftblock; j++) {
                        for (i = 0; i < d_imopVar86[0]; i++) {
                            y0[i][j].real = x_imopVar87[k][j + jj][i].real;
                            y0[i][j].imag = x_imopVar87[k][j + jj][i].imag;
                        }
                    }
                    int _imopVarPre247;
                    int _imopVarPre248;
                    _imopVarPre247 = d_imopVar86[0];
                    _imopVarPre248 = logd_imopVar89[0];
                    cfftz(is_imopVar85, _imopVarPre248, _imopVarPre247, y0, y1);
                    for (j = 0; j < fftblock; j++) {
                        for (i = 0; i < d_imopVar86[0]; i++) {
                            xout_imopVar88[k][j + jj][i].real = y0[i][j].real;
                            xout_imopVar88[k][j + jj][i].imag = y0[i][j].imag;
                        }
                    }
                }
            }
    ...
    int ij2_imopVar107;
    int kk_imopVar108;
    double ap_imopVar109;
#pragma omp parallel default(shared) private(i_imopVar101, j_imopVar102, k_imopVar103, ii_imopVar104, ii2_imopVar105, jj_imopVar106, ij2_imopVar107, kk_imopVar108)
    {
#pragma omp for nowait
        for (i_imopVar101 = 0; i_imopVar101 < dims[2][0]; i_imopVar101++) {
            ii_imopVar104 = (i_imopVar101 + 1 + xstart[2] - 2 + 64 / 2) % 64 - 64 / 2;
            ii2_imopVar105 = ii_imopVar104 * ii_imopVar104;
            for (j_imopVar102 = 0; j_imopVar102 < dims[2][1]; j_imopVar102++) {
                jj_imopVar106 = (j_imopVar102 + 1 + ystart[2] - 2 + 64 / 2) % 64 - 64 / 2;
                ij2_imopVar107 = jj_imopVar106 * jj_imopVar106 + ii2_imopVar105;
                for (k_imopVar103 = 0; k_imopVar103 < dims[2][2]; k_imopVar103++) {
                    kk_imopVar108 = (k_imopVar103 + 1 + zstart[2] - 2 + 64 / 2) % 64 - 64 / 2;
                    indexmap[k_imopVar103][j_imopVar102][i_imopVar101] = kk_imopVar108 * kk_imopVar108 + ij2_imopVar107;
                }
            }
        }
    }
    ap_imopVar109 = -4.0 * 1.0e-6 * 3.141592653589793238 * 3.141592653589793238;
    ex[0] = 1.0;
    double _imopVarPre217_imopVar110;
    _imopVarPre217_imopVar110 = exp(ap_imopVar109);
    ex[1] = _imopVarPre217_imopVar110;
    for (i_imopVar101 = 2; i_imopVar101 <= (6 * (64 * 64 / 4 + 64 * 64 / 4 + 64 * 64 / 4)); i_imopVar101++) {
        ex[i_imopVar101] = ex[i_imopVar101 - 1] * ex[1];
    }
    int ( *_imopVarPre153 );
    ...
    int dir_imopVar111;
    struct stUn_imopVarPre11 ( *x1_imopVar112 )[64][64];
    struct stUn_imopVarPre11 ( *x2_imopVar113 )[64][64];
    dir_imopVar111 = 1;
    x1_imopVar112 = u1;
    x2_imopVar113 = u0;
    dcomplex y0_imopVar114[64][18];
    dcomplex y1_imopVar115[64][18];
    if (dir_imopVar111 == 1) {
        int ( *_imopVarPre225 );
        _imopVarPre225 = dims[0];
        int is;
...
#pragma omp for nowait
        ...
        is_imopVar80 = 1;
        d_imopVar81 = _imopVarPre227;
        x_imopVar82 = x1_imopVar112;
        xout_imopVar83 = x1_imopVar112;
        int logd_imopVar84[3];
#pragma omp parallel default(shared) shared(is_imopVar80)
        {
            int i;
...
#pragma omp for nowait
        ...
        is_imopVar90 = 1;
        d_imopVar91 = _imopVarPre229;
        x_imopVar92 = x1_imopVar112;
        xout_imopVar93 = x2_imopVar113;
        int logd_imopVar94[3];
#pragma omp parallel default(shared) shared(is_imopVar90)
        {
            int i;
...
#pragma omp for nowait
        ...
        is_imopVar75 = _imopVarPre233;
        d_imopVar76 = _imopVarPre232;
        x_imopVar77 = x1_imopVar112;
        xout_imopVar78 = x1_imopVar112;
        int logd_imopVar79[3];
#pragma omp parallel default(shared) shared(is_imopVar75)
        {
            int i;
...
#pragma omp for nowait
        ...
        is_imopVar85 = _imopVarPre241;
        d_imopVar86 = _imopVarPre240;
        x_imopVar87 = x1_imopVar112;
        xout_imopVar88 = x2_imopVar113;
        int logd_imopVar89[3];
#pragma omp parallel default(shared) shared(is_imopVar85)
        {
            int i;
...
#pragma omp master
        ...
        if (dir == 1) {
            ...
            int is_imopVar90;
            int *d_imopVar91;
#pragma omp parallel default(shared) shared(is, is_imopVar80, is_imopVar90)
            {
                int ( *_imopVarPre225 );
#pragma omp master
                {
...
#pragma omp for nowait
            ...
            int *d_imopVar86;
            struct stUn_imopVarPre11 ( *x_imopVar87 )[64][64];
#pragma omp parallel default(shared) shared(is_imopVar75, is, is_imopVar85)
            {
                int ( *_imopVarPre232 );
                int _imopVarPre233;
#pragma omp master
...
#pragma omp barrier
            int i_imopVar96;
            struct stUn_imopVarPre11 ( *u1_imopVar97 )[64][64];
            int *d_imopVar98;
            i_imopVar96 = iter;
            u1_imopVar97 = u2;
            d_imopVar98 = _imopVarPre161;
            int j_imopVar99;
            int q;
            int r;
            int s;
            ...
            chk.imag = 0.0;
#pragma omp for nowait
            for (j_imopVar99 = 1; j_imopVar99 <= 1024; j_imopVar99++) {
                q = j_imopVar99 % 64 + 1;
                int _imopVarPre272;
                ...
                if (_imopVarPre272) {
                    ...
                    if (_imopVarPre274) {
                        ...
                        if (_imopVarPre276) {
                            chk.real = chk.real + u1_imopVar97[s - zstart[0]][r - ystart[0]][q - xstart[0]].real;
                            (chk.imag = chk.imag + u1_imopVar97[s - zstart[0]][r - ystart[0]][q - xstart[0]].imag);
                        }
                    }
                }
            }
...
#pragma omp critical
            {
                sums[i_imopVar96].real += chk.real;
                sums[i_imopVar96].imag += chk.imag;
            }
// #pragma omp dummyFlush CRITICAL_END
#pragma omp single nowait
            {
                sums[i_imopVar96].real = sums[i_imopVar96].real / (double) 262144;
                sums[i_imopVar96].imag = sums[i_imopVar96].imag / (double) 262144;
                double _imopVarPre279;
                double _imopVarPre280;
                _imopVarPre279 = sums[i_imopVar96].imag;
                _imopVarPre280 = sums[i_imopVar96].real;
                printf("T = %5d     Checksum = %22.12e %22.12e\n", i_imopVar96, _imopVarPre280, _imopVarPre279);
            }
#pragma omp master
            {
                if (0 == 1) {
