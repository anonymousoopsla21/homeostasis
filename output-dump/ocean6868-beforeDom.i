...
int simulate_ocean_currents_parallel(double **A, int dim , double tol , int procs) {
    ...
    double **B_imopVarPre10;
    double **A_imopVarPre9;
    double **B_imopVarPre8;
...
#pragma omp single nowait
                ...
                for (i = start; i < end; ++i) {
                    for (j = 1; j < dim - 1; ++j) {
                        old = B[i][j];
                        B_imopVarPre10[i][j] = (B[i][j] + B[i][j - 1] + B[i - 1][j] + B[i][j + 1] + B[i + 1][j]) / 5.0;
                        double _imopVarPre208;
                        double _imopVarPre209;
...
#pragma omp single nowait
                {
                    C_imopVarPre1 = B;
                    A_imopVarPre9 = B_imopVarPre10;
                    B_imopVarPre8 = B;
                }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
                done = diff_imopVarPre2 / (dim * dim) < tol;
                whilePred_imopVarPre0 = !done;
                if (!whilePred_imopVarPre0) {
                    break;
                }
#pragma omp single nowait
                ...
                for (i = start; i < end; ++i) {
                    for (j = 1; j < dim - 1; ++j) {
                        old = B_imopVarPre10[i][j];
                        B_imopVarPre8[i][j] = (B_imopVarPre10[i][j] + B_imopVarPre10[i][j - 1] + B_imopVarPre10[i - 1][j] + B_imopVarPre10[i][j + 1] + B_imopVarPre10[i + 1][j]) / 5.0;
                        double _imopVarPre208;
                        double _imopVarPre209;
                        _imopVarPre208 = C_imopVarPre1[i][j] - old;
                        _imopVarPre209 = fabs(_imopVarPre208);
                        mydiff += _imopVarPre209;
                    }
                }
...
#pragma omp single nowait
                {
                    C_imopVarPre3 = A_imopVarPre9;
                    A = C_imopVarPre1;
                    B_imopVarPre7 = A_imopVarPre9;
                }
// #pragma omp dummyFlush BARRIER_START
#pragma omp barrier
...
#pragma omp single nowait
                ...
                for (i = start; i < end; ++i) {
                    for (j = 1; j < dim - 1; ++j) {
                        old = A[i][j];
                        B_imopVarPre7[i][j] = (A[i][j] + A[i][j - 1] + A[i - 1][j] + A[i][j + 1] + A[i + 1][j]) / 5.0;
                        double _imopVarPre208;
                        double _imopVarPre209;
                        _imopVarPre208 = B_imopVarPre7[i][j] - old;
                        _imopVarPre209 = fabs(_imopVarPre208);
                        mydiff += _imopVarPre209;
                    }
                }
